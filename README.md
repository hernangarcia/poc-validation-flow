...
```bash
flash-serverless-poc$ sam build --use-container
```
...
```bash
flash-serverless-poc$ sam logs -n StockCheckerFunction --stack-name flash-serverless-poc --tail
```
...
```bash
flash-serverless-poc$ pip install -r tests/requirements.txt --user
# unit test
flash-serverless-poc$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
flash-serverless-poc$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name flash-serverless-poc
```
